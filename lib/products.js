...
/**
 * Extrai o modelo base removendo sufixos de variação como (B), (A), etc.
 * @param {string} model - Modelo completo
 * @returns {string} Modelo base
 */
export function getBaseModel(model) {
  // Remove sufixos entre parênteses como (B), (A), (V2), etc.
  return model.replace(/\s*\([^)]+\)\s*$/, '').trim();
}

/**
 * Agrupa produtos por modelo base ou por groupName
 * @param {Array} products - Lista de produtos
 * @returns {Array} Lista de produtos agrupados
 */
export function groupProductsByModel(products) {
  const grouped = {};
  
  products.forEach(product => {
    // Se o produto tem groupName, usa ele como chave de agrupamento
    // Caso contrário, usa o modelo base
    const groupKey = product.groupName || getBaseModel(product.model);
    
    if (!grouped[groupKey]) {
      grouped[groupKey] = {
        baseModel: getBaseModel(product.model),
        displayModel: product.groupName || getBaseModel(product.model),
        groupName: product.groupName,
        variations: [],
        // Usa os dados do primeiro produto como referência
    ...
    grouped[groupKey].variations.push(product);
  });
...
export function searchProducts(query = '', category = null) {
  ...
  // Se houver query, filtra por modelo ou groupName
  if (query && query.trim()) {
    const queryLower = query.toLowerCase();
    filtered = filtered.filter(product => 
      product.model.toLowerCase().includes(queryLower) ||
      (product.groupName && product.groupName.toLowerCase().includes(queryLower))
    );
  }
 ...
 * @param {string} type - Tipo/modelo específico do produto
 * @returns {Object} Arquivos compartilhados da categoria
 */
function getSharedCategoryFiles(category, subcategory = null, type = null) {
  if (!category) return null;
  ...
  try {
    ...
    // Nível 1: Procura no caminho completo subcategory/type/default.json
    if (subcategory && type) {
      const typePath = path.join(categoryPath, subcategory.toLowerCase(), type);
      const typeSharedFile = path.join(typePath, 'default.json');
      
      if (fs.existsSync(typeSharedFile)) {
        const sharedData = fs.readFileSync(typeSharedFile, 'utf8');
    ...
    // Nível 2: Se houver subcategoria, procura na subpasta
    if (subcategory) {
    ...
    // Nível 3: Procura no nível da categoria
...
export function getProductFiles(product) {
  ...
  if (!excludeShared) {
    ...
    const sharedFiles = getSharedCategoryFiles(product.category, subcategory, type);
